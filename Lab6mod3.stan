...
parameters {
  vector[3] beta;           //  intercept
  real<lower=0> sigma;  // error sd for Gaussian likelihood
}
model {
  // Log-likelihood
  target += normal_lpdf(log_weight | beta[1] + beta[2]*log_gest + beta[3]*sex, sigma);
...
generated quantities {
  ...
  for (n in 1:N) {
    real log_weight_hat_n = beta[1] + beta[2]*log_gest[n] + beta[3]*sex[n];
