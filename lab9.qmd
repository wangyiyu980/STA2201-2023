...
```{r}
mod1 <- stan(data = stan_data,file ="../code/lab9_1.stan")
```



```{r}
mod2 <- stan(data = stan_data,file ="../code/lab9_2.stan")
```

```{r}
mod3 <- stan(data = stan_data,file ="../code/lab9_3.stan")
```
...
The level of uncertainty in Model 1 is lower, but it tends to be more focused on specific points. Model 2 has more uncertainty compared to model 3.